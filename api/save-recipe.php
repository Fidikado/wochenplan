...
if (!empty($_FILES['image']) && is_uploaded_file($_FILES['image']['tmp_name'])) {
    ...
    $filename = $base . '-' . substr(uniqid('', true), -6) . '.' . $ext;
    $target = rtrim($config['images_dir'], '/\\') . '/' . $filename;
    ...
    $imagePath = 'rezepte/images/' . $filename;
}

try {
    $recipe = sqlite_recipe_create($config, [
        'title' => trim((string)$input['title']),
        'kategorie' => (string)($input['kategorie'] ?? 'hauptspeise'),
    ...
    if ($imagePath !== '') {
        $uploadedImage = __DIR__ . '/../' . $imagePath;
...
echo json_encode([
    'success' => true,
    'id' => $recipe['id'],
    'message' => "Rezept '" . $recipe['title'] . "' gespeichert",
]);
